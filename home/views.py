...
def add_products(request):
  if request.method == 'POST':
        product = Product(product_admin=request.user)
        product_form = ProductForm(
            request.POST, request.FILES, instance=product)
        if product_form.is_valid():
            product_form.save()
            messages.success(request, "Product is registered.")
            return redirect("product_admin")
  else:
        product_form = ProductForm()

  return render(request, 'admin/add_products.html',context={'product_form' : product_form})
...
def detail_product_view(request, product_id):
    ...
    return render(request, "product_detail.html", {'product': product[0], 'base_template':base_template,'user': request.user.is_product_admin})
...
def edit_product(request, product_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            form = ProductForm(instance=product_to_edit)
    return render(request, "admin/edit_product.html", {'form': form, 'product': product_to_edit})
